...
let nextId = 1;
function file(path: string, status: RoundFile["status"] = "modified", extra: Partial<RoundFile> = {}): RoundFile {
...
describe("FileTree", () => {
  it("renders a row per file with status badges, nested under directories", () => {
    ...
    render(<FileTree files={files} viewed={new Set()} onToggleViewed={noop} onSelect={noop} />);
  ...
  it("toggles viewed state through the dot", () => {
    ...
    const { rerender } = render(
      <FileTree files={files} viewed={new Set()} onToggleViewed={onToggle} onSelect={noop} />,
    );
    ...
    rerender(<FileTree files={files} viewed={new Set(["a.txt"])} onToggleViewed={onToggle} onSelect={noop} />);
    expect(screen.getByRole("button", { name: "Toggle viewed: a.txt" })).toHaveAttribute("aria-pressed", "true");
  });
  ...
  it("reports selection clicks", () => {
    ...
    render(<FileTree files={[file("x.go")]} viewed={new Set()} onToggleViewed={noop} onSelect={onSelect} />);
  ...
  it("renders path text inertly even when it looks like HTML", () => {
    ...
    render(<FileTree files={[file(evil)]} viewed={new Set()} onToggleViewed={noop} onSelect={noop} />);
    expect(screen.getByText(evil)).toBeInTheDocument();
    expect(document.querySelector("img")).toBeNull();
  });

  it("shows the empty message when there are no files", () => {
    render(<FileTree files={[]} viewed={new Set()} onToggleViewed={noop} onSelect={noop} />);
